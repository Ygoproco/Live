...
function c3629090.initial_effect(c)
  ...
  --tohand
	...
	e3:SetTarget(c3629090.atktg)
	e3:SetOperation(c3629090.atkop)
	c:RegisterEffect(e3)
end
...
function c3629090.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_RITUAL)
end
function c3629090.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c3629090.filter,tp,LOCATION_MZONE,0,1,nil) end
end
function c3629090.atkop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c3629090.filter,tp,LOCATION_MZONE,0,nil)
	local tc=g:GetFirst()
	while tc do
		local e1=Effect.CreateEffect(c)
