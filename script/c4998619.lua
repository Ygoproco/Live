...
function c4998619.spfil(c,e,tp)
...
function c4998619.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c4998619.spfil(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingTarget(c4998619.spfil,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	local lc=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if lc>3 then lc=3 end
	if Duel.IsPlayerAffectedByEffect(tp,59822133) then lc=1 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c4998619.spfil,tp,LOCATION_GRAVE,0,1,lc,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,g:GetCount(),0,0)
end
function c4998619.thfil(c)
...
function c4998619.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(c4998619.thfil,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(4998619,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,c4998619.thfil,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
