...
function c59496924.initial_effect(c)
	...
	e2:SetTarget(c59496924.reptg)
	e2:SetValue(c59496924.repval)
	e2:SetOperation(c59496924.repop)
	c:RegisterEffect(e2)
	--spsummon
	...
	e3:SetCondition(c59496924.spcon)
	e3:SetCost(c59496924.spcost)
	e3:SetTarget(c59496924.sptg)
	e3:SetOperation(c59496924.spop)
	c:RegisterEffect(e3)
end

function c59496924.filter(c,tp)
...
function c59496924.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c59496924.filter,1,nil,tp) and Duel.IsExistingMatchingCard(c59496924.drfil,tp,LOCATION_ONFIELD,0,1,nil,eg) end
	if Duel.SelectYesNo(tp,aux.Stringid(59496924,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
		local g=Duel.SelectMatchingCard(tp,c59496924.drfil,tp,LOCATION_ONFIELD,0,1,1,nil,eg)
...
function c59496924.repval(e,c)
	return c59496924.filter(c,e:GetHandlerPlayer())
end
