...
function c43266605.filter(c,tp)
...
function c43266605.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c43266605.filter,1,nil,tp) and not e:GetHandler():IsStatus(STATUS_DESTROY_CONFIRMED) end
	return Duel.SelectYesNo(tp,aux.Stringid(43266605,1))
end
function c43266605.repval(e,c)
	return c43266605.filter(c,e:GetHandlerPlayer())
end
...
function c43266605.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c43266605.spfilter,1,nil,tp)
end
function c43266605.spfilter(c,tp)
