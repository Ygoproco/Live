...
function c85004150.xyzcon(e,c,og)
	...
	return Duel.CheckXyzMaterial(c,c85004150.ovfilter2,7,2,2,og)
end
function c85004150.xyzop(e,tp,eg,ep,ev,re,r,rp,c,og)
	if og then
		...
		local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
		local ct=-ft
		local b1=Duel.CheckXyzMaterial(c,c85004150.ovfilter2,7,2,2,og)
		...
		if b2 and (not b1 or Duel.SelectYesNo(tp,aux.Stringid(85004150,0))) then
			...
			local mg=Duel.SelectXyzMaterial(tp,c,c85004150.ovfilter2,7,2,2)
			c:SetMaterial(mg)
			Duel.Overlay(c,mg)
		end
	end
