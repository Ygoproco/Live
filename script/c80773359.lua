...
function c80773359.initial_effect(c)
  --synchro summon
	...
	e5:SetCondition(c80773359.atkcon)
	e5:SetValue(3000)
	c:RegisterEffect(e5)
end
...
function c80773359.lvtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c80773359.lvfil(chkc,lv) end
  if chk==0 then return Duel.IsExistingTarget(c80773359.lvfil,tp,LOCATION_GRAVE,0,1,nil,lv) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
  Duel.SelectTarget(tp,c80773359.lvfil,tp,LOCATION_GRAVE,0,1,1,nil,lv)
end
...
function c80773359.atkfil(c)
  return not c:IsType(TYPE_SYNCHRO)
end
function c80773359.atkcon(e,tp,eg,ep,ev,re,r,rp)
  ...
  local c=e:GetHandler()
	return (ph==PHASE_DAMAGE or ph==PHASE_DAMAGE_CAL) and Duel.GetAttacker()==c 
	  ...
	  and not c:GetMaterial():IsExists(c80773359.atkfil,1,nil)
end