...
function c6114.initial_effect(c)
	--Destroy
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(6114,0))
	e1:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE+CATEGORY_ATKCHANGE)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SUMMON_SUCCESS)
	e1:SetProperty(EFFECT_FLAG_DAMAGE_STEP)
	e1:SetCondition(c6114.condition)
	e1:SetTarget(c6114.target)
	e1:SetOperation(c6114.operation)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e3:SetDescription(aux.Stringid(6114,1))
	e3:SetCategory(CATEGORY_DAMAGE)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e3:SetCode(EVENT_PHASE+PHASE_END)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCountLimit(1)
	e3:SetCondition(c6114.hvcon)
	e3:SetOperation(c6114.hvop)
	c:RegisterEffect(e3)
end

function c6114.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_HAND)
end
function c6114.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c6114.hvop(e,tp,eg,ep,ev,re,r,rp)
