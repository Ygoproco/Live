...
function c2530830.initial_effect(c)
	...
	e3:SetCondition(c2530830.spcon)
	e3:SetTarget(c2530830.sptg)
	e3:SetOperation(c2530830.spop)
	c:RegisterEffect(e3)
end
...
function c2530830.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c2530830.spfilter(c,e,tp)
	return c:IsCode(18963306) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c2530830.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c2530830.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c2530830.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c2530830.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
end
function c2530830.spop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
