...
function c46035545.atkcfil(c)
  return c:IsSetCard(0x10af) and c:IsDiscardable()
end
function c46035545.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c46035545.atkcfil,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,c46035545.atkcfil,1,1,REASON_COST+REASON_DISCARD)
end
function c46035545.atkfil(c)
  return c:IsFaceup() and c:IsSetCard(0xaf) and c:IsLevelBelow(6)
end
function c46035545.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c46035545.atkfil(chkc) end
  if chk==0 then return Duel.IsExistingTarget(c46035545.atkfil,tp,LOCATION_MZONE,0,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
  Duel.SelectTarget(tp,c46035545.atkfil,tp,LOCATION_MZONE,0,1,1,nil)
end
...
function c46035545.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  if chk==0 then return Duel.IsExistingTarget(c46035545.thfil1,tp,LOCATION_MZONE,0,1,nil) 
	and (Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7))
	and Duel.IsExistingMatchingCard(c46035545.thfil2,tp,LOCATION_EXTRA,0,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
  local g=Duel.SelectTarget(tp,c46035545.thfil1,tp,LOCATION_MZONE,0,1,1,nil)
...
function c46035545.thop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 then
	...
	local g=Duel.SelectMatchingCard(tp,c46035545.thfil2,tp,LOCATION_EXTRA,0,1,ct,nil)
	local pc=g:GetFirst()
	while pc do
