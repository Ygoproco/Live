...
function c65959844.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsDestructable,tp,0,LOCATION_ONFIELD,1,nil) and Duel.IsExistingMatchingCard(c65959844.descfil,tp,LOCATION_MZONE,0,1,nil) end
...
function c65959844.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c65959844.descfil,tp,LOCATION_MZONE,0,1,1,nil)
	local rc=g:GetFirst()
...
function c65959844.retcon(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffect(65959844)==0 then
		e:Reset()
		return false
	else
