...
function c7714344.cfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x9f) and not c:IsCode(7714344)
end
function c7714344.damcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c7714344.cfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c7714344.cfilter,tp,LOCATION_GRAVE,0,1,1,nil)
...
function c7714344.damop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.RegisterFlagEffect(tp,7714344,RESET_PHASE+PHASE_END,0,1)
end
function c7714344.damval(e,re,val,r,rp,rc)
	local tp=e:GetHandlerPlayer()
	if Duel.GetFlagEffect(tp,7714344)==0 or bit.band(r,REASON_BATTLE)==0 then return val end
	Duel.ResetFlagEffect(tp,7714344)
	return 0
end