...
--Shiranui Sage
function c7133.initial_effect(c)
  --Special Summon
  local e1=Effect.CreateEffect(c)
  e1:SetDescription(aux.Stringid(7133,0))
  e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
  e1:SetType(EFFECT_TYPE_IGNITION)
  e1:SetRange(LOCATION_MZONE)
  e1:SetCountLimit(1,7133)
  e1:SetCost(c7133.spcost1)
  e1:SetTarget(c7133.sptg1)
  e1:SetOperation(c7133.spop1)
  c:RegisterEffect(e1)
  --Special Summon
  local e2=Effect.CreateEffect(c)
  e2:SetDescription(aux.Stringid(7133,1))
  e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
  e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
  e2:SetCode(EVENT_REMOVE)
  e2:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DELAY)
  e2:SetCountLimit(1,7133+1)
  e2:SetTarget(c7133.sptg2)
  e2:SetOperation(c7133.spop2)
  c:RegisterEffect(e2)
end

function c7133.spcost1(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c7133.spfil1(c,e,tp)
  return c:IsRace(RACE_ZOMBIE) and c:IsType(TYPE_TUNER) and c:GetDefence()==0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c7133.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c7133.spfil1,tp,LOCATION_DECK,0,1,nil,e,tp) end
  Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c7133.spop1(e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
  local g=Duel.SelectMatchingCard(tp,c7133.spfil1,tp,LOCATION_DECK,0,1,1,nil,e,tp)
  if g:GetCount()>0 then
	Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
  end
end

function c7133.spfil2(c,e,tp)
  return c:IsSetCard(0xd9) and not c:IsCode(7133) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c7133.spex(c)
...
function c7133.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
  local g=Duel.SelectTarget(tp,c7133.spfil2,tp,LOCATION_REMOVED,0,1,ft,nil,e,tp)
