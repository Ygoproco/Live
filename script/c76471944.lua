...
function c76471944.cfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP)
end
function c76471944.descon(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_GRAVE,0,1,nil,TYPE_SPELL+TYPE_TRAP)
end
function c76471944.fil(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsDestructable()
end
function c76471944.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and c76471944.fil(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c76471944.fil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c76471944.fil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
