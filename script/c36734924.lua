...
function c36734924.initial_effect(c)
	...
	e2:SetTarget(c36734924.tdtg)
	e2:SetOperation(c36734924.tdop)
	c:RegisterEffect(e2)
end
...
function c36734924.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c36734924.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c36734924.thfilter,tp,LOCATION_DECK,0,1,nil) end
...
function c36734924.tdfilter(c)
	return c:IsSetCard(0xe0) and c:IsAbleToDeck()
end
function c36734924.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c36734924.tdfilter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.IsExistingTarget(c36734924.tdfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,c36734924.tdfilter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c36734924.tdop(e,tp,eg,ep,ev,re,r,rp)
