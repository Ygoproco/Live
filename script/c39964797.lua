...
function c39964797.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(39964797,0))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetCode(EVENT_FREE_CHAIN)
	e3:SetHintTiming(0,0x1c0+TIMING_MAIN_END)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c39964797.spcon)
	e3:SetTarget(c39964797.sptg)
	e3:SetOperation(c39964797.spop)
	c:RegisterEffect(e3)
	--
	...
	e2:SetCondition(c39964797.spcon2)
	e2:SetTarget(c39964797.sptg2)
	e2:SetOperation(c39964797.spop2)
	c:RegisterEffect(e2)
end
...
function c39964797.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c39964797.spcon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c39964797.spfil(c,e,tp)
		return c:IsSetCard(0xea) and not c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c39964797.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
		if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c39964797.spfil(chkc,e,tp) end
		if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingTarget(c39964797.spfil,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectTarget(tp,c39964797.spfil,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
end
function c39964797.spop2(e,tp,eg,ep,ev,re,r,rp)
