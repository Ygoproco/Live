...
function c7025.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCode2(c,91998119,7024,true,true)
	--spsummon condition
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCode(EFFECT_SPSUMMON_CONDITION)
	e1:SetValue(c7025.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetCondition(c7025.spcon)
	e2:SetOperation(c7025.spop)
	c:RegisterEffect(e2)
	--negate
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(7025,0))
	e3:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY)
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetCode(EVENT_CHAINING)
	e3:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c7025.discon)
	e3:SetCost(c7025.discost)
	e3:SetTarget(c7025.distg)
	e3:SetOperation(c7025.disop)
	c:RegisterEffect(e3)
	--summon 2 banished monsters
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(7025,1))
	e4:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e4:SetType(EFFECT_TYPE_QUICK_O)
	e4:SetCode(EVENT_FREE_CHAIN)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCost(c7025.rmcost)
	e4:SetTarget(c7025.rmtg)
	e4:SetOperation(c7025.rmop)
	c:RegisterEffect(e4)
end
function c7025.splimit(e,se,sp,st)
...
function c7025.spcon(e,c)
	...
	local g1=Duel.GetMatchingGroup(c7025.spfilter,tp,LOCATION_ONFIELD,0,nil,91998119)
	local g2=Duel.GetMatchingGroup(c7025.spfilter,tp,LOCATION_ONFIELD,0,nil,7024)
...
function c7025.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local g1=Duel.GetMatchingGroup(c7025.spfilter,tp,LOCATION_ONFIELD,0,nil,91998119)
	local g2=Duel.GetMatchingGroup(c7025.spfilter,tp,LOCATION_ONFIELD,0,nil,7024)
	g1:Merge(g2)
...
function c7025.distg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c7025.disop(e,tp,eg,ep,ev,re,r,rp)
...
function c7025.rmfilter(c,e,tp,code)
	return c:IsCode(code) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function c7025.rmcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c7025.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g1=Duel.SelectMatchingCard(tp,c7025.rmfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp,91998119)
	local g2=Duel.SelectMatchingCard(tp,c7025.rmfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp,7024)
	g1:Merge(g2)
