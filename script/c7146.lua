...
function c7146.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(7146,0))
	...
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetDescription(aux.Stringid(7146,1))
	...
	e3:SetCondition(c7146.indcon)
	e3:SetOperation(c7146.indop)
	c:RegisterEffect(e3)
	--atkdown
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(7146,2))
	e4:SetCategory(CATEGORY_ATKCHANGE)
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_DAMAGE_STEP_END)
	e4:SetRange(LOCATION_PZONE)
	e4:SetCondition(c7146.atkcon)
	e4:SetOperation(c7146.atkop)
	c:RegisterEffect(e4)
	--salvage
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(7146,3))
	e5:SetCategory(CATEGORY_TOHAND)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e5:SetCode(EVENT_SPSUMMON_SUCCESS)
	e5:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e5:SetCondition(c7146.thcon)
	e5:SetTarget(c7146.thtg)
	e5:SetOperation(c7146.thop)
	c:RegisterEffect(e5)
	--lp
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(7146,4))
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e6:SetCode(EVENT_BATTLE_DESTROYING)
	e6:SetCondition(aux.bdocon)
	e6:SetOperation(c7146.lpop)
	c:RegisterEffect(e6)
	--pendulum
	local e7=Effect.CreateEffect(c)
	e7:SetDescription(aux.Stringid(7146,5))
	e7:SetCategory(CATEGORY_DESTROY)
	e7:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e7:SetCode(EVENT_DESTROYED)
	e7:SetProperty(EFFECT_FLAG_DELAY)
	e7:SetCondition(c7146.pencon)
...
function c7146.syncon(e,c,tuner,mg)
	...
	local g2=nil
	if mg then
		g1=mg:Filter(c7146.matfilter1,nil,c)
		g2=mg:Filter(c7146.matfilter2,nil,c)
	else
		g1=Duel.GetMatchingGroup(c7146.matfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
		g2=Duel.GetMatchingGroup(c7146.matfilter2,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)
	end
	...
	if not pe then
		return g1:IsExists(c7146.synfilter,1,nil,c,lv,g2)
	else
		return c7146.synfilter(pe:GetOwner(),c,lv,g2)
	end
end
function c7146.syntg(e,tp,eg,ep,ev,re,r,rp,chk,c,tuner,mg)
	...
	if tuner then
		...
		local tuner=nil
		if not pe then
			local t1=g1:FilterSelect(tp,c7146.synfilter,1,1,nil,c,lv,g2)
			tuner=t1:GetFirst()
		else
			tuner=pe:GetOwner()
...
function c7146.synop(e,tp,eg,ep,ev,re,r,rp,c,tuner,mg)
...
function c7146.indop(e,tp,eg,ep,ev,re,r,rp)
...
function c7146.thop(e,tp,eg,ep,ev,re,r,rp)
...
function c7146.lpop(e,tp,eg,ep,ev,re,r,rp)
	Duel.SetLP(1-tp,math.ceil(Duel.GetLP(1-tp)/2))
end
function c7146.pencon(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(r,REASON_EFFECT+REASON_BATTLE)~=0 and e:GetHandler():IsPreviousLocation(LOCATION_MZONE)
end
function c7146.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7) end
end
function c7146.penop(e,tp,eg,ep,ev,re,r,rp)
