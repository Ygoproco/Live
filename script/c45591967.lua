...
function c45591967.dmfil(c)
  return c:IsFaceup() and c:IsType(TYPE_PENDULUM)
end
function c45591967.dmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c45591967.dmfil,tp,LOCATION_EXTRA,0,1,nil) end
	local dam=Duel.GetMatchingGroupCount(c45591967.dmfil,tp,LOCATION_EXTRA,0,nil)*300
...
function c45591967.dmop(e,tp,eg,ep,ev,re,r,rp)
  if not e:GetHandler():IsRelateToEffect(e) then return end
	...
	local d=Duel.GetMatchingGroupCount(c45591967.dmfil,tp,LOCATION_EXTRA,0,nil)*300
...
function c45591967.desfil(c)
	return (c:GetSequence()==6 or c:GetSequence()==7) and c:IsDestructable()
end
function c45591967.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsLocation(LOCATION_SZONE) and c45591967.desfil(chkc) end
  if chk==0 then return Duel.IsExistingTarget(c45591967.desfil,tp,LOCATION_SZONE,LOCATION_SZONE,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
  local g=Duel.SelectTarget(tp,c45591967.desfil,tp,LOCATION_SZONE,LOCATION_SZONE,1,1,nil)
