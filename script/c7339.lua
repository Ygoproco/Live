...
function c7339.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	...
	e0:SetCondition(c7339.fscon)
	e0:SetOperation(c7339.fsop)
	c:RegisterEffect(e0)
	--spsummon condition
	...
	e2:SetDescription(aux.Stringid(7339,0))
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c7339.eqcon)
	e2:SetTarget(c7339.eqtg)
	e2:SetOperation(c7339.eqop)
	c:RegisterEffect(e2)
	...
	e3:SetCode(7339)
	e3:SetTarget(c7339.mattg)
	c:RegisterEffect(e3)
end
function c7339.filter1(c)
	return c:IsFusionSetCard(0xe1)
end
function c7339.filter2(c)
	return c:IsType(TYPE_NORMAL)
end
function c7339.exfilter(c)
	return c:IsHasEffect(7339)
end
function c7339.fscon(e,g,gc,chkfnf)
	if g==nil then return true end
	local f1=c7339.filter1
	local f2=c7339.filter2
	local chkf=bit.band(chkfnf,0xff)
	local exg=Duel.GetMatchingGroup(c7339.exfilter,e:GetHandlerPlayer(),LOCATION_SZONE,0,nil)
	exg:Merge(g)
...
function c7339.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
	local f1=c7339.filter1
	local f2=c7339.filter2
	local chkf=bit.band(chkfnf,0xff)
	local exg=Duel.GetMatchingGroup(c7339.exfilter,tp,LOCATION_SZONE,0,nil)
	exg:Merge(eg)
...
function c7339.eqcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()~=tp
end
function c7339.eqfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_EFFECT) and (c:IsControler(tp) or c:IsAbleToChangeControler())
end
function c7339.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c7339.eqfilter(chkc,tp) and chkc~=e:GetHandler() end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c7339.eqfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c7339.eqfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,e:GetHandler(),tp)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
end
function c7339.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		...
		e2:SetValue(c7339.eqlimit)
		e2:SetReset(RESET_EVENT+0x1fe0000)
		tc:RegisterEffect(e2)
	else Duel.SendtoGrave(tc,REASON_EFFECT) end
end
function c7339.eqlimit(e,c)
	return e:GetOwner()==c
end
function c7339.mattg(e,c)
