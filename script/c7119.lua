...
function c7119.initial_effect(c)
  ...
  local e2=Effect.CreateEffect(c)
	...
	e3:SetDescription(aux.Stringid(7119,1))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_CHAIN_SOLVING)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,7119+1)
	e3:SetRange(LOCATION_GRAVE)
	e3:SetCondition(c7119.spscon)
	e3:SetTarget(c7119.spstg)
	e3:SetOperation(c7119.spsop)
	c:RegisterEffect(e3)
end
...
function c7119.spscon(e,tp,eg,ep,ev,re,r,rp)
...
function c7119.spstg(e,tp,eg,ep,ev,re,r,rp,chk)
  local c=e:GetHandler()
  if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
  Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c7119.spsop(e,tp,eg,ep,ev,re,r,rp)
  local c=e:GetHandler()
