...
function c34318086.thcfil(c)
	return (c:IsSetCard(0x20a2) or c:IsSetCard(0x30a2)) and not c:IsCode(34318086)
end
function c34318086.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c34318086.thcfil,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,c34318086.thcfil,1,1,nil)
	Duel.Release(g,REASON_COST)
end
function c34318086.thfil(c)
	return c:IsRace(RACE_SPELLCASTER) and c:IsAbleToHand()
end
function c34318086.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c34318086.thfil,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,0)
end
function c34318086.thop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local g=Duel.SelectMatchingCard(tp,c34318086.thfil,tp,LOCATION_DECK,0,1,1,nil)
...
function c34318086.spfil(c,e,tp)
	return c:IsRace(RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c34318086.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c34318086.spfil,tp,LOCATION_HAND,0,1,nil,e,tp) end
...
function c34318086.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c34318086.spfil,tp,LOCATION_HAND,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
		local tc=g:GetFirst()
