...
function c6120.initial_effect(c)
	--pendulum summon
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c6120.spcon)
	e3:SetTarget(c6120.sptg)
	e3:SetOperation(c6120.spop)
	c:RegisterEffect(e3)
	--disable
	...
	e4:SetCountLimit(1)
	e4:SetTarget(c6120.distg)
	e4:SetOperation(c6120.disop)
	c:RegisterEffect(e4)
end
...
function c6120.regop1(e,tp,eg,ep,ev,re,r,rp)
	if rp==tp or not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) or eg:GetCount()~=1
		or not eg:IsExists(c6120.regfilter,1,nil) then
...
function c6120.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsExistingMatchingCard(c6120.penfilter,tp,LOCATION_EXTRA,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c6120.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
		local g=Duel.SelectMatchingCard(tp,c6120.penfilter,tp,LOCATION_EXTRA,0,1,1,nil)
		local tc=g:GetFirst()
		if tc then
...
function c6120.disfilter(c)
	return c:IsFaceup() and c:GetSummonLocation()==LOCATION_EXTRA and not c:IsDisabled()
end
function c6120.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c6120.disfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c6120.disfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c6120.disfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,g,1,0,0)
end
function c6120.disop(e,tp,eg,ep,ev,re,r,rp)
