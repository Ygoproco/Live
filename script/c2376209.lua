...
function c2376209.initial_effect(c)
	...
	e1:SetCondition(c2376209.adcon)
	e1:SetTarget(c2376209.adtg)
	e1:SetOperation(c2376209.adop)
	c:RegisterEffect(e1)
	--spsummon
...
function c2376209.adcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function c2376209.adtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() end
	if chk==0 then return Duel.IsExistingTarget(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
function c2376209.adop(e,tp,eg,ep,ev,re,r,rp)
