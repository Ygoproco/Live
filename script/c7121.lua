...
function c7121.initial_effect(c)
  ...
  --Recover
	...
	e3:SetDescription(aux.Stringid(7121,1))
	e3:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_CHAIN_SOLVING)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,7121+1)
	e3:SetRange(LOCATION_GRAVE)
	e3:SetCondition(c7121.svcon)
	e3:SetCost(c7121.svcost)
	e3:SetTarget(c7121.svtg)
	e3:SetOperation(c7121.svop)
	c:RegisterEffect(e3)
end
...
function c7121.svcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c7121.costfilter,1,nil) end
	local sg=Duel.SelectReleaseGroup(tp,c7121.costfilter,1,1,nil)
	Duel.Release(sg,REASON_COST)
end
function c7121.svtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,e:GetHandler(),1,0,0)
end
function c7121.svop(e,tp,eg,ep,ev,re,r,rp)
