...
function c11493868.atkfil(c)
	return c:IsFaceup() and c:IsSetCard(0x9b)
end
function c11493868.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c11493868.atkfil(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c11493868.atkfil,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,c11493868.atkfil,tp,LOCATION_MZONE,0,1,1,nil)
end
