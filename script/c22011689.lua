...
function c22011689.initial_effect(c)
  --
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c22011689.rctg)
	e3:SetOperation(c22011689.rcop)
	c:RegisterEffect(e3)
end
...
function c22011689.rcfil(c,eg)
  return eg and eg:IsContains(c) and c:IsDestructable()
end
function c22011689.rctg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  local qg=e:GetHandler():GetEquipGroup()
  if chkc then return chkc:IsLocation(LOCATION_SZONE) and chkc:IsControler(tp) and qg and c22011689.rcfil(chkc,qg) end
  if chk==0 then return Duel.IsExistingTarget(c22011689.rcfil,tp,LOCATION_SZONE,0,1,nil,qg) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
  local g=Duel.SelectTarget(tp,c22011689.rcfil,tp,LOCATION_SZONE,0,1,1,nil,qg)
  Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
  local rc=g:GetFirst():GetBaseAttack()
  Duel.SetTargetPlayer(tp)
  Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,rc)
end
function c22011689.rcop(e,tp,eg,ep,ev,re,r,rp)
  local tc=Duel.GetFirstTarget()
