...
function c3129635.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		if a:IsControler(tp) then return d and a~=e:GetHandler() and bit.band(a:GetOriginalType(),TYPE_TRAP)~=0 and a:IsStatus(STATUS_OPPO_BATTLE)
		else return d and d~=e:GetHandler() and bit.band(d:GetOriginalType(),TYPE_TRAP)~=0 and d:IsStatus(STATUS_OPPO_BATTLE) end
	end
