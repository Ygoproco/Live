...
function c58604027.initial_effect(c)
	...
	e5:SetCountLimit(1)
	e5:SetCondition(c58604027.con)
	e5:SetTarget(c58604027.tg)
	e5:SetOperation(c58604027.op)
	c:RegisterEffect(e5)
	--Draw
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(58604027,2))
...
function c58604027.atkfil(c)
	return c:IsSetCard(0x40) and c:IsType(TYPE_MONSTER)
end
function c58604027.atkval(e,c)
	return Duel.GetMatchingGroupCount(c58604027.atkfil,c:GetControler(),LOCATION_GRAVE,0,nil)*1000
end

function c58604027.con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c58604027.fil(c)
	return c:IsSetCard(0x40) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c58604027.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c58604027.fil,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,0)
end
function c58604027.op(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(tp,c58604027.fil,tp,LOCATION_GRAVE,0,1,1,nil)
...
function c58604027.drfilter(c)
	return c:IsSetCard(0x40) and c:IsType(TYPE_MONSTER) and not c:IsPublic()
end
function c58604027.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp) and Duel.IsExistingMatchingCard(c58604027.drfilter,tp,LOCATION_HAND,0,1,nil) end
...
function c58604027.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(p,c58604027.drfilter,p,LOCATION_HAND,0,1,63,nil)
	if g:GetCount()>0 then
