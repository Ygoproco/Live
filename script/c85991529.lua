...
function c85991529.initial_effect(c)
  c:EnableReviveLimit()
	...
	e4:SetCondition(c85991529.condition)
	e4:SetCost(c85991529.cost)
	e4:SetTarget(c85991529.target)
	e4:SetOperation(c85991529.activate)
	c:RegisterEffect(e4)
	--to hand
...
function c85991529.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(c85991529.spfilter,c:GetControler(),LOCATION_HAND,0,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local sg=g:SelectWithSumGreater(tp,Card.GetLevel,10)
	Duel.Remove(sg,POS_FACEUP,REASON_COST)
end

function c85991529.condition(e,tp,eg,ep,ev,re,r,rp)
	return re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
function c85991529.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c85991529.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c85991529.activate(e,tp,eg,ep,ev,re,r,rp)
