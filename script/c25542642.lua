...
function c25542642.initial_effect(c)
	...
	e1:SetOperation(c25542642.operation)
	c:RegisterEffect(e1)
	--Special Summon
	...
	e2:SetCountLimit(1,25542642)
	e2:SetCost(c25542642.spcost)
	e2:SetTarget(c25542642.sptg)
	e2:SetOperation(c25542642.spop)
	c:RegisterEffect(e2)
end
...
function c25542642.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		e1:SetCondition(c25542642.rcon)
...
function c25542642.spfil(c,e,tp)
...
function c25542642.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c25542642.spfil(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c25542642.spfil,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c25542642.spfil,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
