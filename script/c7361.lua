...
--Igknight Unite
--Script by nekrozar
function c7361.initial_effect(c)
	--Activate
	...
	e1:SetCountLimit(1,7361+EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c7361.target)
	e1:SetOperation(c7361.activate)
	c:RegisterEffect(e1)
end
function c7361.desfilter1(c)
...
function c7361.activate(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 then
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,c7361.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
