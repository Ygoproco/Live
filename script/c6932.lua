...
--Amorphage Irritum
function c6932.initial_effect(c)
	--Pendulum Summon
	...
	e4:SetDescription(aux.Stringid(6932,0))
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e4:SetRange(LOCATION_PZONE)
	e4:SetCountLimit(1)
	e4:SetCode(EVENT_PHASE+PHASE_STANDBY)
	e4:SetCondition(c6932.descon)
	e4:SetTarget(c6932.destg)
	e4:SetOperation(c6932.desop)
	c:RegisterEffect(e4)
	--Remove
	...
	e3:SetCondition(c6932.chaincon)
	e3:SetTarget(c6932.rmtarget)
	...
	e2:SetTarget(c6932.splimit)
	c:RegisterEffect(e2)
end

function c6932.descon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c6932.destg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c6932.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFacedown() or not c:IsRelateToEffect(e) then return end
	if Duel.CheckReleaseGroup(tp,Card.IsReleasableByEffect,1,c) and Duel.SelectYesNo(tp,aux.Stringid(6932,1)) then
		local g=Duel.SelectReleaseGroup(tp,Card.IsReleasableByEffect,1,1,c)
		Duel.Release(g,REASON_EFFECT)
	else Duel.Destroy(c,REASON_EFFECT) end
end

function c6932.cfilter(c)
