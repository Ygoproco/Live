...
function c7218.spop1(e,tp,eg,ep,ev,re,r,rp)
  ...
  local g=Duel.SelectMatchingCard(tp,c7218.spfil1,LOCATION_HAND,0,1,1,nil,e,tp)
...
function c7218.spop2(e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
  local g=Duel.SelectMatchingCard(tp,c7218.spfil2,LOCATION_DECK,0,1,1,nil,e,tp)
  if g:GetCount()>0 then
	Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
  end
end