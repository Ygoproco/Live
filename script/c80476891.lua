...
function c80476891.initial_effect(c)
	...
	e1:SetTarget(c80476891.atktg)
	e1:SetOperation(c80476891.atkop)
	c:RegisterEffect(e1)
	--Destroy Replace
...
function c80476891.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	if chk==0 then return tc:IsType(TYPE_DUAL) and tc~=e:GetHandler() end
	Duel.SetTargetCard(tc)
end
function c80476891.atkop(e,tp,eg,ep,ev,re,r,rp)
...
function c80476891.filter(c,tp)
...
function c80476891.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c80476891.filter,1,nil,tp) and Duel.IsExistingMatchingCard(c80476891.drfil,tp,LOCATION_ONFIELD,0,1,nil,eg) end
	if Duel.SelectYesNo(tp,aux.Stringid(80476891,1)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
		local g=Duel.SelectMatchingCard(tp,c80476891.drfil,tp,LOCATION_ONFIELD,0,1,1,nil,eg)
...
function c80476891.repval(e,c)
	return c80476891.filter(c,e:GetHandlerPlayer())
end
