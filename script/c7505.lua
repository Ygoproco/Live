...
function c7505.initial_effect(c)
	...
	e1:SetCondition(c7505.spcon)
	e1:SetTarget(c7505.sptg)
	e1:SetOperation(c7505.spop)
	c:RegisterEffect(e1)
	--Synchro Summon
	...
	e2:SetCondition(c7505.sccon)
	e2:SetTarget(c7505.sctg)
	e2:SetOperation(c7505.scop)
	c:RegisterEffect(e2)
	--atk change
	...
	e3:SetCondition(c7505.con)
	e3:SetTarget(c7505.tg)
	e3:SetOperation(c7505.op)
	c:RegisterEffect(e3)
end

function c7505.spcon(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	local b=false
	if PHASE_BATTLE_START then
		b=(ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE)
	else
		b=(ph==PHASE_BATTLE or (ph==PHASE_DAMAGE and not Duel.IsDamageCalculated()))
	end
	return ep==tp and bit.band(r,REASON_BATTLE+REASON_EFFECT)~=0 and b
end
function c7505.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,1,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c7505.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c7505.sccon(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	local b=false
	if PHASE_BATTLE_START then
		b=(ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE)
...
function c7505.mfilter(c)
	return c:IsAttribute(ATTRIBUTE_WIND)
end
function c7505.sctg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		local mg=Duel.GetMatchingGroup(c7505.mfilter,tp,LOCATION_MZONE,0,nil)
...
function c7505.scop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c7505.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsLocation(LOCATION_GRAVE) and r==REASON_SYNCHRO
end
function c7505.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDiscardDeck(tp,1) end
end
function c7505.op(e,tp,eg,ep,ev,re,r,rp)
