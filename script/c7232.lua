...
function c7232.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(7232,0))
	e2:SetCategory(CATEGORY_DAMAGE+CATEGORY_RECOVER)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_DAMAGE_STEP_END)
	e2:SetTarget(c7232.damtg)
	e2:SetOperation(c7232.damop)
	c:RegisterEffect(e2)
...
function c7232.damop(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e2:SetTarget(c7232.sptg)
	e2:SetOperation(c7232.spop)
	e2:SetReset(RESET_EVENT+0x1fc0000)
	c:RegisterEffect(e2)
	bc:AddCounter(0x39,1)
end

function c7232.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c7232.spop(e,tp,eg,ep,ev,re,r,rp)
