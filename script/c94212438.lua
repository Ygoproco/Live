...
function c94212438.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsExistingMatchingCard(c94212438.cfilter1,tp,LOCATION_ONFIELD,0,1,nil,31893528) then
	...
	elseif not Duel.IsExistingMatchingCard(c94212438.cfilter1,tp,LOCATION_ONFIELD,0,1,nil,67287533) then
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(94212438,1))
		local g=Duel.SelectMatchingCard(tp,Card.IsCode,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,67287533)
		if g:GetCount()>0 then
			Duel.MoveToField(g:GetFirst(),tp,tp,LOCATION_SZONE,POS_FACEUP,true)
		end
	elseif not Duel.IsExistingMatchingCard(c94212438.cfilter1,tp,LOCATION_ONFIELD,0,1,nil,94772232) then
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(94212438,1))
		local g=Duel.SelectMatchingCard(tp,Card.IsCode,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,94772232)
		if g:GetCount()>0 then
			Duel.MoveToField(g:GetFirst(),tp,tp,LOCATION_SZONE,POS_FACEUP,true)
		end
	elseif not Duel.IsExistingMatchingCard(c94212438.cfilter1,tp,LOCATION_ONFIELD,0,1,nil,30170981) then
		Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(94212438,1))
