...
function c58988903.cfilter(c,e,tp)
	return c:IsSetCard(0xba) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost() and Duel.IsExistingTarget(c58988903.spfil,tp,LOCATION_GRAVE,0,1,c,e,tp)
end
...
function c58988903.spfil(c,e,tp)
...
function c58988903.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c58988903.spfil(chkc,e,tp) end
	if chk==0 then return Duel.IsExistingTarget(c58988903.spfil,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c58988903.spfil,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
