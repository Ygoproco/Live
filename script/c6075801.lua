...
function c6075801.initial_effect(c)
	...
	e3:SetCondition(c6075801.immcon)
	e3:SetValue(c6075801.efilter)
	c:RegisterEffect(e3)
	--Special Summon
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(6075801,1))
...
function c6075801.eqfil(c,ec)
	return c:IsRace(RACE_DRAGON) and (c:GetLevel()==7 or c:GetLevel()==8)
end
function c6075801.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c6075801.eqfil,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e:GetHandler()) end
...
function c6075801.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c6075801.eqfil,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,c)
	local tc=g:GetFirst()
...
function c6075801.immcon(e)
	return e:GetHandler():GetEquipCount()>0
end
function c6075801.efilter(e,re)
