...
function c83443619.spfil(c,e,tp)
	return c:IsSetCard(0xea) and c:IsType(TYPE_TUNER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c83443619.spcfil(c)
	return c:IsFaceup() and c:IsDestructable()
end
function c83443619.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and c83443619.spcfil(chkc) end
	if chk==0 then
		...
		e:SetLabel(loc)
		return Duel.IsExistingTarget(c83443619.spcfil,tp,loc,0,1,nil)
			and Duel.IsExistingMatchingCard(c83443619.spfil,tp,LOCATION_DECK,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c83443619.spcfil,tp,e:GetLabel(),0,1,1,nil)
...
function c83443619.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 then
		...
		local g=Duel.SelectMatchingCard(tp,c83443619.spfil,tp,LOCATION_DECK,0,1,1,nil,e,tp)
		if g:GetCount()>0 then
			...
			e3:SetTarget(c83443619.splimit)
			e3:SetReset(RESET_PHASE+PHASE_END)
			Duel.RegisterEffect(e3,tp)
		end
	end
end
function c83443619.splimit(e,c,tp,sumtp,sumpos)
...
function c83443619.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c83443619.thfil,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,0,0)
end
function c83443619.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c83443619.thfil,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
