...
function c15180041.initial_effect(c)
  c:EnableReviveLimit()
	...
	e5:SetCountLimit(1)
	e5:SetCondition(c15180041.negcon)
	e5:SetTarget(c15180041.negtg)
	e5:SetOperation(c15180041.negop)
	c:RegisterEffect(e5)
	--Special Summon
	...
	e6:SetCondition(c15180041.spcon2)
	e6:SetTarget(c15180041.sptg2)
	e6:SetOperation(c15180041.spop2)
	c:RegisterEffect(e6)
end
...
function c15180041.negcon(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) then return false end
	return re:IsActiveType(TYPE_SPELL) and re:IsHasType(EFFECT_TYPE_ACTIVATE) and Duel.IsChainNegatable(ev)
end
function c15180041.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end
function c15180041.negop(e,tp,eg,ep,ev,re,r,rp)
...
function c15180041.spfilter(c,e,tp)
	return c:IsSetCard(0xe7) and not c:IsCode(15180041) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function c15180041.sptg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c15180041.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND+LOCATION_DECK)
end
function c15180041.spop2(e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c15180041.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,tp)
