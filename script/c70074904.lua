...
function c70074904.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and e:GetHandler():IsStatus(STATUS_OPPO_BATTLE)
end
function c70074904.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		return (t==c and a:IsAbleToRemove())
			or (a==c and t~=nil and t:IsAbleToRemove()) and a:GetControler()~=t:GetControler()
	end
