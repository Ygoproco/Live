...
function c29491334.initial_effect(c)
	...
	e3:SetCondition(c29491334.spcon)
	e3:SetCost(c29491334.spcost)
	e3:SetTarget(c29491334.sptg)
	e3:SetOperation(c29491334.spop)
	c:RegisterEffect(e3)
end

function c29491334.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,29491334+1,0,0x4011,2000,2400,6,RACE_MACHINE,ATTRIBUTE_DARK) end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c29491334.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,29491334+1,0,0x4011,2000,2400,6,RACE_MACHINE,ATTRIBUTE_DARK) then return end
	local token=Duel.CreateToken(tp,29491334+1)
