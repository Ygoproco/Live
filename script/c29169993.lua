...
function c29169993.thfil(c)
  return c:IsSetCard(0x9f) and c:IsLevelBelow(3) and c:IsAbleToHand()
end
function c29169993.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c29169993.thfil(chkc) end
  if chk==0 then return e:GetHandler():IsAbleToGrave() and Duel.IsExistingTarget(c29169993.thfil,tp,LOCATION_GRAVE,0,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
  local g=Duel.SelectTarget(tp,c29169993.thfil,tp,LOCATION_GRAVE,0,1,1,nil)
...
function c29169993.spcfil(c)
	return c:IsSetCard(0x9f) and c:IsPreviousLocation(LOCATION_HAND)
end
function c29169993.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return eg:IsExists(c29169993.spcfil,1,e:GetHandler()) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
