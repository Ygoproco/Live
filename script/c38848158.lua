...
function c38848158.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(e:GetLabel()) and chkc:IsControler(tp) and chkc~=e:GetHandler() and c38848158.desfilter1(chkc) end
	if chk==0 then
		...
		e:SetLabel(loc)
		return Duel.IsExistingTarget(c38848158.desfilter1,tp,loc,0,1,e:GetHandler())
			and Duel.IsExistingMatchingCard(c38848158.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c38848158.desfilter1,tp,e:GetLabel(),0,1,1,e:GetHandler())
