...
function c47819246.initial_effect(c)
	...
	e1:SetOperation(c47819246.activate)
	c:RegisterEffect(e1)
end
...
function c47819246.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c47819246.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c47819246.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c47819246.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		e3:SetCountLimit(1)
		e3:SetCondition(c47819246.atkcon)
		e3:SetCost(c47819246.atkcost)
		e3:SetOperation(c47819246.atkop)
		c:RegisterEffect(e3)
	end
end
...
function c47819246.atkcon(e,tp,eg,ep,ev,re,r,rp)
...
function c47819246.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToGraveAsCost() end
	local tc=e:GetHandler():GetEquipTarget()
	Duel.SetTargetCard(tc)
	Duel.SendtoGrave(e:GetHandler(),REASON_COST)
end
function c47819246.atkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
