...
function c98153934.damfil(c,tp)
	return c:IsLocation(LOCATION_MZONE) and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x10df) and c:IsControler(tp)
end
function c98153934.damcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then return false end
	local g=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
	if not g or not g:IsExists(c98153934.damfil,1,nil,tp) then return false end
	return ep~=tp and c:GetFlagEffect(98153934)~=0
end
...
function c98153934.repfilter(c,tp)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsOnField() and c:IsControler(tp) and c:IsReason(REASON_EFFECT+REASON_BATTLE)
end
function c98153934.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	return Duel.SelectYesNo(tp,aux.Stringid(98153934,1))
end
