...
function c6404.initial_effect(c)
	...
	e1:SetTarget(c6404.thtg)
	e1:SetOperation(c6404.thop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e4:SetDescription(aux.Stringid(6404,1))
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_GRAVE)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetCost(c6404.cost)
	e4:SetTarget(c6404.target)
	e4:SetOperation(c6404.operation)
	c:RegisterEffect(e4)
end

function c6404.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(Card.IsFacedown,tp,LOCATION_SZONE,LOCATION_SZONE,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,g:GetCount(),0,0)
end
function c6404.thop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsFacedown,tp,LOCATION_SZONE,LOCATION_SZONE,nil)
	Duel.SendtoHand(g,nil,REASON_EFFECT)
end

function c6404.cost(e,tp,eg,ep,ev,re,r,rp,chk)
