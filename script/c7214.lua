...
--Berry Magician Girl
function c7214.initial_effect(c)
  --Search
  local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(7214,0))
	e3:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_SUMMON_SUCCESS)
	e3:SetTarget(c7214.thtg)
	e3:SetOperation(c7214.thop)
	c:RegisterEffect(e3)
	--Special Summon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(7214,1))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_BE_BATTLE_TARGET)
	e1:SetCountLimit(1,EFFECT_COUNT_CODE_SINGLE)
	e1:SetTarget(c7214.sptg)
	e1:SetOperation(c7214.spop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	e2:SetType(EFFECT_TYPE_QUICK_O)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCode(EVENT_BECOME_TARGET)
	e2:SetCondition(c7214.spcon)
	c:RegisterEffect(e2)
end
...
function c7214.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c7214.thfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c7214.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c7214.spop(e,tp,eg,ep,ev,re,r,rp)
  ...
  if Duel.ChangePosition(c,POS_FACEUP_DEFENCE,POS_FACEDOWN_DEFENCE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)>0 then
	...
	local g=Duel.SelectMatchingCard(tp,c7214.spfil,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
